    ...
    def process_monitor(self):
        while True in list(map(lambda x: self.thread_dict[x][0].is_alive(),self.thread_dict)):    
            ...
            for proc in self.thread_dict:
                worked = "Работает" if self.thread_dict[proc][0].is_alive() else "Закончил работу"
                print(f"ID: {self.thread_dict[proc][0].name}| Статус: {worked} | Текущая задача: {self.thread_dict[proc][1]}")
            print("===="*20)
        else:
            return 0
    ...
    def parse_subcatalogs(self,driver: webdriver.Chrome, subcatalogs:list[tuple], test=False, key_stat = ""):
        ...
        for s in subcatalogs:
            ...
            try:
                ...
                if test:
                    logging.info(f"--------> Работа потока <--------- \n ==========> добавил {s} ========> \
                             {list(map(lambda x: x[0], tovar_data))} в {title}\n Исходный список: {tovar_data} \n\n СПИСОК ССЫЛОК: {links}")
                ...
                if tovar_data != []:
                    if self.main_data.get(title) != None:
                        ...
                        tovar_data = []                      
                    if test:
                        logging.info(f"Пересохраненный main_data[{title}] работа идет с {s}: {self.main_data[title]}")
